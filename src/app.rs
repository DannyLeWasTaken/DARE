...
pub trait App {
	...
	fn frame(&mut self, _ctx: Context, _ifc: phobos::InFlightContext) -> Result<phobos::sync::submit_batch::SubmitBatch<phobos::domain::All>> {
...
impl Runner {
	pub fn new(
		...
		make_settings: impl Fn(phobos::AppBuilder<winit::window::Window>) ->
	...
	fn frame<E: App + 'static>(&mut self, app: &mut E, window: &winit::window::Window) -> Result<()> {
		let ctx = self.make_context();
		let frame = self.vk.frame.as_mut().unwrap();
		let surface = self.vk.surface.as_ref().unwrap();
		futures::executor::block_on(
			frame.new_frame(self.vk.execution_manager.clone(), window, surface,
